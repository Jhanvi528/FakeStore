...
const Product = ({ product }) => {
	...
	const handleQtyChange = action => {
		if (action === "inc") {
			if (options.quantity < product.variants.find(({ id }) => id === options.variantId).inventory_quantity)
				setOptions({
				...
				<div className={styles.details}>
					...
					<div className={styles.selection}>
						...
						<div style={{ display: "flex", justifyContent: "end" }}>
							<div className={styles.qty}>
								<button className={styles.qtybtn} onClick={() => handleQtyChange("dec")}>
									-
								</button>
								<span className={styles.ticker}>{options.quantity}</span>
								<button className={styles.qtybtn} onClick={() => handleQtyChange("inc")}>
									+
								</button>
							</div>
						</div>
					...
					<div className={styles.tabs}>
						...
						<div className="tab-content">
							<p style={{ color: "grey", fontStyle: "italic" }}>{product.description}</p>
						</div>
					</div>
				</div>
			</div>
